...
The computational models are all based on signal detection theory (Green & Swets, 1966). It is assumed that participants select a binary discrimination response $RR about a stimulus $SR.
Both $S$ and $R$ can be either -1 or 1. $R$ is considered correct if $S=R$.
In addition, we assume that there are $KR different levels of stimulus discriminability
...
The models assume that the stimulus generates normally distributed sensory evidence $xR with mean $S\times d_k/2R
and variance of 1. The sensory evidence $xR is compared to a decision
criterion $c} to generate a discrimination response
$R}, which is 1, if $x} exceeds $c} and -1 else.
...
- confidence criterion $\theta_{-1,1}},$\theta_{-1,2}},
..., $\theta_{-1,L-1}}, $\theta_{1,1}},  $\theta_{1,2}},...,
$\theta_{1,L-1}} ($L}: number of confidence categories available for confidence ratings)
