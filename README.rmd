...
especially, when there are multiple models, several difficulty conditions, and/or  several confidence categories.  Depending on the model and the number of experimental conditions, fitting one participant and model combination may take up to 20-30 minutes on a modern 2.8GHz CPU.
...
the data (which is not the case though in the demo dataset), the function `fitConfModels` can be used to estimate meta-d′/d′. The arguments `.data` and `.parallel=TRUE` are identical to the arguments in `fitConfModels`. The argument `model` offers the user the choice between two model specificationsEither "ML" to use the original model specification used by Maniscalco and Lau (2012, 2014) or "F" to use the model specification by Fleming (2017)'s Hmetad method. 

```         
MetaDs <- fitMetaDprime(data = subset(MaskOri, diffCond == "33.3"), 
                        model="ML", .parallel = TRUE)
```

## Contributing to the package 
The package is under active development. We aim to include other models that are published in the literature in the package.
