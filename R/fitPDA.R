...
function(ratings, stimulus, correct, condition){
    ...
    temp <- expand.grid(minD =c(0,.5),
                        ...
                        tauRange = seq(1,5,length.out=4),  # range of rating criteria stimulus B  #  position of the most liberal confidence criterion with respect to thet
                        a = c(1/2, 1, 2, 4)) # pproportional signal increase between decision and confidence
    ...
    logL <- apply(inits, MARGIN = 1, 
                  function(p) try(llPDA(p, N_SA_RA, N_SA_RB, N_SB_RA,N_SB_RB, nRatings, nCond), silent = TRUE))
