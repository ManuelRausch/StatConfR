...
function(ratings, stimulus, correct, condition){
    ...
    temp <- expand.grid(minD = c(0,.5),
                        ...
                        sigma = c(1/2, 1,2,4), # noise parameter
                        w = seq(.1, .9, length.out=4), 
                        a = c(1/2, 1, 2, 4)) # range of stimulus and stimulus-unrelated variance at d=1
    
    inits <- data.frame(matrix(data=NA, nrow= nrow(temp), ncol = nCond + nRatings*2 + 2))
    ...
    inits[,(nCond + nRatings*2 + 2)] <- log(temp$a) 
    
    logL <- apply(inits, MARGIN = 1, 
                  function(p) try(llCEVvarS(p, N_SA_RA, N_SA_RB, N_SB_RA,N_SB_RB, nRatings, nCond), silent = TRUE))
    ...
    if(class(fit) != "try-error"){
      ...
      res[,paste("cA",1:(nRatings-1), sep="")] <- 
        c(as.vector(fit$par[nCond+nRatings-1] - 
                      rev(cumsum(c(exp(fit$par[(nCond+1):(nCond+nRatings-2)]))))),  
      ...
      res$a <- exp(fit$par[nCond + nRatings*2 + 2])
